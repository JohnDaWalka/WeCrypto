...
(function () {
  ...
  const PROXY_ORIGIN = 'http://localhost:3010';
  // Tauri exposes window.__TAURI__; Electron used file:// protocol
  const IS_TAURI     = typeof window.__TAURI__ !== 'undefined';
  const IS_ELECTRON  = IS_TAURI || window.location.protocol === 'file:';
  ...
  // s2.ripple.com, xrplcluster.com, api.mainnet-beta.solana.com, etc.
  ...
  const mode = IS_TAURI ? 'Tauri (A+B→proxy, C→direct)' : IS_ELECTRON ? 'Electron (A+B→proxy, C→direct)' : 'browser (all→proxy)';
