...
Python handles all heavy math:
  ...
  * Grid-search threshold optimisation (entryThreshold x minAgreement)
...
OUTER_ORBITAL_WEIGHTS = dict(
    ...
    vwap=0.05,       # BNB inner (5x bias); 33-43% WR globally
...
def calc_hma(data: np.ndarray, period: int = 16) -> np.ndarray:
    """Hull MA: WMA(2xWMA(n/2) − WMA(n), floor(sqrtn)). Nearly lag-free."""
    if len(data) < 4:
...
def build_signal_model(candles: list[dict], coin: str = "") -> Optional[dict]:
    ...
    keys = list(sv.keys())
    # Apply per-coin orbital bias: inner + outer orbital weights x coinBias
