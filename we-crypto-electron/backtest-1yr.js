#!/usr/bin/env node
// ================================================================
// WECRYPTO — 1-Year Walk-Forward Backtest
// Fetches up to 365 days of real Binance.US 5m candles (paginated),
// runs the exact same indicator logic used in the live app,
// and validates every signal against actual outcomes.
//
// Key fixes vs backtest-runner.js:
//   1. Paginated fetch  — Binance allows max 1000 bars/request;
//      this walks backwards in 1000-bar chunks to collect a full year.
//   2. Sliding window   — model always sees the last LIVE_WINDOW=300
//      candles, matching what the live app actually uses and keeping
//      compute O(n) instead of O(n²).
//   3. Monthly breakdown — win-rate/equity per calendar month.
//
// Usage:  node backtest-1yr.js
//         node backtest-1yr.js --coin BTC
//         node backtest-1yr.js --days 180
// ================================================================
'use strict';
...
const LIVE_WINDOW = 300;   // exact candles the live app sends to buildSignalModel
...
const BACKTEST_FILTER_OVERRIDES = {
  BTC:  { h1:{et:0.54,ma:0.65}, h5:{et:0.62,ma:0.75}, h10:{et:0.70,ma:0.83}, h15:{et:0.78,ma:0.92} },
  ETH:  { h1:{et:0.54,ma:0.65}, h5:{et:0.62,ma:0.75}, h10:{et:0.70,ma:0.83}, h15:{et:0.78,ma:0.92} },
  SOL:  { h1:{et:0.50,ma:0.58}, h5:{et:0.58,ma:0.68}, h10:{et:0.66,ma:0.78}, h15:{et:0.74,ma:0.88} },
  XRP:  { h1:{et:0.36,ma:0.42}, h5:{et:0.42,ma:0.50}, h10:{et:0.50,ma:0.58}, h15:{et:0.58,ma:0.66} },
  DOGE: { h1:{et:0.66,ma:0.75}, h5:{et:0.74,ma:0.85}, h10:{et:0.82,ma:0.92}, h15:{et:0.90,ma:1.00} },
  BNB:  { h1:{et:0.46,ma:0.58}, h5:{et:0.54,ma:0.68}, h10:{et:0.62,ma:0.78}, h15:{et:0.70,ma:0.88} },
  HYPE: { h1:{et:0.50,ma:0.58}, h5:{et:0.58,ma:0.68}, h10:{et:0.66,ma:0.78}, h15:{et:0.74,ma:0.88} },
};
const DEFAULT_FILTERS = {
  h1:{et:0.42,ma:0.55}, h5:{et:0.50,ma:0.65}, h10:{et:0.58,ma:0.75}, h15:{et:0.66,ma:0.85}
};
...
function calcHurstExponent(returns,lagsMin=2,lagsMax=19){
  if(!returns||returns.length<lagsMax+2)return 0.5;
  const xs=[],ys=[];
  for(let lag=lagsMin;lag<=lagsMax;lag++){
    const diffs=[];
    for(let i=lag;i<returns.length;i++)diffs.push(returns[i]-returns[i-lag]);
    const tau=calcStd(diffs);
    if(tau>0){xs.push(Math.log(lag));ys.push(Math.log(tau));}
  }
...
function calcVarianceRatio(returns,lag=5,window=100){
  if(!returns||returns.length<Math.max(window,lag*3))return 1;
  const x=returns.slice(-window);
  const v1=calcStd(x)**2;
  if(v1<=0)return 1;
  const ksum=[];
  for(let i=0;i+lag<=x.length;i+=lag){
    let s=0;
    for(let j=0;j<lag;j++)s+=x[i+j];
    ksum.push(s);
  }
  if(ksum.length<2)return 1;
  const vk=calcStd(ksum)**2;
  return vk/(lag*v1);
}
function calcRollingEntropy(returns,window=50,bins=10){
  if(!returns||returns.length<window)return 1;
  const x=returns.slice(-window);
...
function computeAdvancedRegime(candles){
  const closes=candles.map(c=>c.c);
  const returns=calcReturns(closes);
  const hurst=calcHurstExponent(returns);
  const vr=calcVarianceRatio(returns,5,100);
  const entropyNorm=clamp(calcRollingEntropy(returns,50,10),0,1);

  const hurstTrend=clamp((hurst-0.5)*2,0,1);
  const vrTrend=clamp((vr-1)*1.5,0,1);
  const entropySignal=1-entropyNorm;

  const regimeScore=clamp(0.4*hurstTrend+0.4*vrTrend+0.2*entropySignal,0,1);
  const trendWeight=regimeScore;
  const meanRevWeight=1-regimeScore;
  const label=regimeScore>=0.6?'trend':regimeScore<=0.3?'mean_reversion':'mixed';

  return{hurst,vr,entropy:entropyNorm,regimeScore,trendWeight,meanRevWeight,label};
}
function scoreBucket(a){return a>=0.4?'strong':a>=0.25?'moderate':a>=0.1?'light':'neutral';}
function signalFromScore(s){const a=Math.abs(s);if(a<0.20)return'neutral';return s>0?(a>0.55?'strong_bull':'bullish'):(a>0.55?'strong_bear':'bearish');}
...
function buildSignalModel(candles) {
  ...
  const regime = computeAdvancedRegime(candles);
  ...
  const trendKeys = new Set(['ema','vwap','obv','volume','momentum','persistence','structure','macd','adx','ichimoku']);
  const meanRevKeys = new Set(['rsi','bands','stochrsi','williamsR','mfi']);
  const keys=Object.keys(sv);
  const tw=keys.reduce((s,k)=>{
    const base=COMPOSITE_WEIGHTS[k]||0;
    const factor=trendKeys.has(k)
      ? (0.6+0.8*regime.trendWeight)
      : meanRevKeys.has(k)
        ? (0.6+0.8*regime.meanRevWeight)
        : 1;
    return s+base*factor;
  },0)||1;
  const composite=keys.reduce((s,k)=>{
    const base=COMPOSITE_WEIGHTS[k]||0;
    const factor=trendKeys.has(k)
      ? (0.6+0.8*regime.trendWeight)
      : meanRevKeys.has(k)
        ? (0.6+0.8*regime.meanRevWeight)
        : 1;
    return s+sv[k]*base*factor;
  },0)/tw;
  // ADX gate: suppress signal in flat markets; amplify to realistic confidence range
  const adxGate = adxR.adx < 20 ? Math.max(0.25, adxR.adx / 20) : 1.0;
  const score=clamp(composite * SCORE_AMPLIFIER * adxGate,-1,1),agr=summarizeAgreement(sv);

  return{score,signal:signalFromScore(score),absScore:Math.abs(score),agreement:agr.agreement,conflict:agr.conflict,coreScore:score,structureBias:structure.signal,structureZone:structure.zone,persistenceScore:persistence.signal,vwapDev,emaCross,rsi,mom,atrPct,signalVector:sv,regimeScore:regime.regimeScore,regimeLabel:regime.label,hurst:regime.hurst,varianceRatio:regime.vr,entropy:regime.entropy};
}
...
// ── Paginated Binance.US fetch ────────────────────────────────────
// Walks backwards from now in 1000-bar chunks to cover DAYS_BACK days.
async function fetchBinancePaginated(symbol, days) {
  const barMs    = 5 * 60 * 1000;          // 5-min bar in ms
  const totalMs  = days * 24 * 60 * 60 * 1000;
  const startTs  = Date.now() - totalMs;
  const allBars  = [];
  let endTs      = Date.now();
  let retries    = 0;

  while (endTs > startTs) {
    const url = `https://api.binance.us/api/v3/klines?symbol=${symbol}&interval=5m&limit=1000&endTime=${endTs}`;
    let resp;
    try {
      resp = await httpGet(url);
    } catch(e) {
      if (++retries > 3) throw new Error(`Binance.US fetch failed after retries: ${e.message}`);
    ...
    if (resp.status === 429 || resp.status === 418) {
      // Rate-limited — back off 3 seconds
    ...
    // Move window back — use oldest bar's open time - 1ms
...
async function fetchKrakenPaginated(sym, days) {
  const KRAKEN_PAIR = { BTC:'XXBTZUSD',ETH:'XETHZUSD',SOL:'SOLUSD',XRP:'XXRPZUSD',DOGE:'XDGUSD',BNB:'BNBUSD',HYPE:'HYPEUSD' };
  const pair = KRAKEN_PAIR[sym]; if (!pair) throw new Error(`No Kraken pair: ${sym}`);
  ...
  if (!Array.isArray(rows)) throw new Error(`Kraken bad response`);
  return rows.map(r=>({t:+r[0]*1000,o:+r[1],h:+r[2],l:+r[3],c:+r[4],v:+r[6]}));
}

async function fetchCandles(coin, days) {
  const errs = [];
  try { const d = await fetchBinancePaginated(coin.binSym, days); if (d.length > 60) return d; } catch(e) { errs.push(`BinUS: ${e.message}`); }
  try { const d = await fetchKrakenPaginated(coin.sym, days);     if (d.length > 60) return d; } catch(e) { errs.push(`Kraken: ${e.message}`); }
...
function printReport(sym, bt, candleCount) {
  ...
  SHORT_HORIZON_MINUTES.forEach(h => {
    ...
    if (r.monthly?.length) {
      console.log(` │ Monthly win rates (newest first):`);
      r.monthly.slice(0,12).forEach(m => {
...
async function main() {
  ...
  const totalBars = DAYS_BACK * 288;

  console.log('');
  console.log('╔══════════════════════════════════════════════════════════════════════════╗');
  console.log('║  WeCrypto-P-1.1 — 1-Year Walk-Forward Backtest                         ║');
  console.log(`║  ${DAYS_BACK} days × 288 bars/day = ${totalBars.toLocaleString()} × 5m candles per coin (paginated)     ║`);
  ...
  for (const coin of coins) {
    process.stdout.write(`\n  Fetching ${coin.sym} (${DAYS_BACK} days)... `);
    let candles;
    try {
      candles = await fetchCandles(coin, DAYS_BACK);
  ...
  // ── Summary table ─────────────────────────────────────────────
  console.log('\n\n' + '═'.repeat(76));
  console.log(' 1-YEAR SUMMARY — Win Rates by Coin & Horizon');
  ...
  // ── Save report ───────────────────────────────────────────────
  const outPath = path.join(__dirname, 'backtest-1yr-report.json');
  try {
    fs.writeFileSync(outPath, JSON.stringify({
      generatedAt: new Date().toISOString(), daysBack: DAYS_BACK,
