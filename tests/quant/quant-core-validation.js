/**
 * ================================================================
 * Quantitative Core Validation Tests
...
// Load modules (simulated IIFE exports to window)
...
test('Isotonic Regression - monotonicity', () => {
    const utils = {
        isotonicRegression: function (x, y) {
            const n = x.length;
            let ghat = [...y];
            let changed = true;
    ...
    const result = utils.isotonicRegression(x, y);
...
test('Entropy calculation', () => {
    const probs = [0.25, 0.25, 0.25, 0.25];  // uniform = high entropy
    let h = 0;
    for (let p of probs) {
...
test('HMM Viterbi path (simple)', () => {
    ...
    // Observation: HIGH state has higher mean
    const obs = [5, 4.8, 0.2, 0.1, 5.1, 4.9];  // alternates: HIGH, HIGH, LOW, LOW, HIGH, HIGH

    // Simple Viterbi for single observation
    let maxState = obs[0] > 2.5 ? 0 : 1;  // HIGH or LOW
...
test('Kalman Filter - state update', () => {
    ...
    assert(x > 0 && x < 1, 'State should move toward observation');
    assert(P < P_pred, 'Covariance should decrease after update');
});

test('Kalman Filter - trend tracking', () => {
    // Deterministic uptrend with bounded oscillation
    ...
    // Simple level + velocity tracking
    for (let i = 1; i < series.length; i++) {
        const innovation = series[i] - level;
        velocity = 0.1 * innovation;  // estimate velocity from innovation
        level += 0.5 * innovation;
        levels.push(level);
    }

    // Check that extracted level is roughly trending up
...
test('Hurst - trending series (H > 0.5)', () => {
    ...
    // Consecutive returns should be highly autocorrelated (trending property)
    const returns = [];
...
test('Hurst - mean-reverting series (H < 0.5)', () => {
    // Anti-persistent return process to emulate mean-reversion
    const meanReverting = [];
    let ret = 1;
    let price = 50;
    for (let i = 0; i < 120; i++) {
...
test('Calibration - well-calibrated predictions', () => {
    ...
    const outcomes = [1, 1, 0, 1, 0, 1, 1];  // ~60% win rate
...
test('Calibration - overconfident predictions', () => {
    ...
    const outcomes = [1, 0, 0, 0, 1];  // only 40% win rate
...
test('ADWIN - detects shift', () => {
    ...
    // Add data from distribution 1
    for (let i = 0; i < 30; i++) {
        detector.add(i % 10);
    }

    // Add data from distribution 2 (shifted)
    for (let i = 0; i < 20; i++) {
        detector.add((i % 10) + 20);
    }

    // Later data should have higher mean
...
test('Segmentation - chi-square significance', () => {
    // 10 wins, 5 losses = significant edge (expected 7.5 each under null)
    const n = 15;
    const wins = 10;
    const expected = n / 2;
    ...
    assert(chisq > 1, 'Chi-square should be > 1 for significant edge');
});

// ──────────────────────────────────────────────────────────────
// 8. TRADE JOURNAL TESTS
// ──────────────────────────────────────────────────────────────
